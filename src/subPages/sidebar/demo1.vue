<template>
  <page-wraper>
    <view class="page-sidebar-demo1">
      <wd-sidebar v-model="active" @change="handleChange">
        <wd-sidebar-item v-for="(item, index) in categories" :key="index" :value="index" :label="item.label" />
      </wd-sidebar>
      <scroll-view class="page-sidebar-demo1__content" scroll-y scroll-with-animation :scroll-top="scrollTop" :throttle="false" @scroll="onScroll">
        <view v-for="(item, index) in categories" :key="index" class="page-sidebar-demo1__category">
...
onMounted(() => {
  getRect('.page-sidebar-demo1__category', true).then((rects) => {
