...
description: 迁移组件 Markdown 文档到新规范。用户要求校验文档与组件 API 一致性、按 demo 规则重排示例或合并 Attributes/Options 列并迁移最低版本标记时调用。
---
...
- 用户要求“检查文档与组件实现是否一致”。
- 用户要求“迁移某组件文档到新规范”。
- 用户要求“按 create-demo-page 规则整理 Demo 顺序”。
...
- 使用 `create-demo-page` 作为默认基线顺序，但允许按组件语义做自适应调整。
- 若文档存在介绍性章节（如适用场景、使用前说明、平台差异、注意事项），应保留在 Demo 分组之前，不强制重排。
- 当进入 Demo 能力展示区时，优先采用以下顺序：
  1. 组件类型（Type）
  2. 组件状态（State）
  3. 组件变体（Variant）
  4. 组件样式（Style）
...
- “检查 upload 文档的 attributes、emits、slots、methods”
